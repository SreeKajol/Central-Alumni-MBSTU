<aside id="sidebar" class="sidebar expanded">
    <div class="h-full flex flex-col">
        ...
        <div class="p-4 border-b border-primary-700">
            <div class="flex items-center space-x-3">
                ...
                <div class="flex-1 min-w-0">
                    <p class="sidebar-text text-sm font-semibold text-white truncate">{{ auth()->user()->name }}</p>
                    <p class="sidebar-text text-xs text-primary-200">{{ auth()->user()->role->label() }}</p>
                </div>
            </div>
        </div>
        @endauth
        ...
        <nav class="flex-1 overflow-y-auto py-4">
            ...
            @if(auth()->check() && auth()->user()->alumniProfile)
                <a href="{{ route('recommendations.index') }}" class="sidebar-link {{ request()->routeIs('recommendations.*') ? 'active' : '' }}">
                    ...
                    <span class="sidebar-text">Smart Recommendations</span>
                </a>
            @endif

            <a href="{{ route('analytics.career') }}" class="sidebar-link {{ request()->routeIs('analytics.*') ? 'active' : '' }}">
                <svg class="sidebar-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if(auth()->check() && auth()->user()->isAlumni() && !auth()->user()->alumniProfile)
                <a href="{{ route('alumni.create') }}" class="sidebar-link bg-primary-700">
                    <svg class="sidebar-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if(auth()->check() && auth()->user()->alumniProfile)
                <a href="{{ route('alumni.show', auth()->user()->alumniProfile) }}" class="sidebar-link {{ request()->routeIs('alumni.show') ? 'active' : '' }}">
                    <svg class="sidebar-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
